...
function Dashboard({ session, onLogout }: { session: UserSession; onLogout: () => void }) {
  ...
  const section1Customers = useMemo(() => customerChipData(yardRows.map(customerNameFromReceipt)), [yardRows]);
  const filteredYardRows = useMemo(() => yardRows.filter((r) => section1Customer === 'ALL' || customerNameFromReceipt(r) === section1Customer), [yardRows, section1Customer]);
  ...
  return <div className="bay-dashboard"><div className="storm-bg" />
    ...
    <main className="content-layout">
      <div className="main-col">
        <section className="dash-panel"><PanelTitle title="Section 1 — In-Yard FULL Equipment" count={`${filteredYardRows.length} rows`} /><CustomerChips customers={section1Customers} selected={section1Customer} onSelect={setSection1Customer} /><table className="dash-table"><thead><tr><th>Equipment #</th><th>RN #</th><th>Check-In (PT)</th><th>Time in Yard</th><th>Customer</th><th>Location</th><th>Assignee</th><th>Action</th></tr></thead><tbody>{filteredYardRows.length ? filteredYardRows.map((r, i) => <tr key={r.id || r.receiptId || i}><td>{r.containerNo || r.trailerNo || '—'}</td><td className="purple-text">{receiptNumber(r)}</td><td>{fmtDate(r.inYardTime || r.createdTime || r.createdAt)}</td><td>{timeInYard(r.inYardTime || r.createdTime || r.createdAt)}</td><td>{customerNameFromReceipt(r)}</td><td><DockSelect value={locationByRow[`yard-${r.id || r.receiptId || i}`] || `DOCK${String(51+i).padStart(2,'0')}`} onChange={(value) => setLocationByRow((p) => ({ ...p, [`yard-${r.id || r.receiptId || i}`]: value }))} /></td><td><select className="inline-select" value={assigneeByRow[`yard-${r.id || r.receiptId || i}`] || defaultAssignee} onChange={(e) => setAssigneeByRow((p) => ({...p, [`yard-${r.id || r.receiptId || i}`]: e.target.value}))}>{cottonAssignees.map((u) => <option key={u.userId || u.username} value={displayUser(u)}>{displayUser(u)}</option>)}</select></td><td><button type="button" className="tiny-assign" disabled={assigningRowId === `yard-${r.id || r.receiptId || i}`} onClick={() => reviewAssignment({ id: `yard-${r.id || r.receiptId || i}`, workType: 'Receipt', reference: receiptNumber(r), customer: customerNameFromReceipt(r), status: r.status || 'IN_YARD', orderType: 'Inbound', defaultAssignee: assigneeByRow[`yard-${r.id || r.receiptId || i}`] || defaultAssignee, historyCount: 0, rule: 'In-yard receipt' })}>{assigningRowId === `yard-${r.id || r.receiptId || i}` ? 'Assigning...' : 'Assign'}</button></td></tr>) : <tr><td colSpan={8} className="empty-cell">No in-yard full equipment rows matched this customer.</td></tr>}</tbody></table></section>
